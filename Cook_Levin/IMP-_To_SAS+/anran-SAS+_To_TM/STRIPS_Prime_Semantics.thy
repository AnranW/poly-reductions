...
*)
theory STRIPS_Prime_Semantics
  imports "STRIPS_Prime_Representation"
    "$AFP/Verified_SAT_Based_AI_Planning/List_Supplement"
...
definition  is_serial_solution_for_problem
  where "is_serial_solution_for_problem \<Pi> \<pi>
    \<equiv> (goal_of \<Pi>) \<subseteq>\<^sub>m execute_serial_plan (initial_of \<Pi>) \<pi>
      \<and> list_all (\<lambda>op. op \<in> (operators_of \<Pi>)) \<pi>"
  ...
  where "is_parallel_solution_for_problem \<Pi> \<pi>
    \<equiv> (strips_problem.goal_of \<Pi>) \<subseteq>\<^sub>m execute_parallel_plan
      ...
      \<and> list_all (\<lambda>ops. list_all (\<lambda>op.
         op \<in> (strips_problem.operators_of \<Pi>)) ops) \<pi>"
...
lemma is_parallel_solution_for_problem_operator_set:
  ...
  assumes "is_parallel_solution_for_problem \<Pi> \<pi>"
    and "ops \<in> set \<pi>"
    and "op \<in> set ops"
  shows "op \<in>  ((\<Pi>)\<^sub>\<O>)"
  proof -
    have "\<forall>ops \<in> set \<pi>. \<forall>op \<in> set ops. op \<in> (strips_problem.operators_of \<Pi>)"
      using assms(1)
...
lemma execute_serial_plan_split:
  ...
  proof (induction \<pi>\<^sub>1 arbitrary: s)
    ...
    moreover have "STRIPS_Prime_Representation.is_operator_applicable_in s op"
      using Cons.prems(1)
      unfolding are_all_operators_applicable_def list_all_iff
      by fastforce
    ultimately show ?case
...
lemma embedding_lemma_iii:
  fixes \<Pi>:: "'a strips_problem"
  assumes "\<forall>op \<in> set \<pi>. op \<in>  ((\<Pi>)\<^sub>\<O>)"
  shows "\<forall>ops \<in> set (embed \<pi>). \<forall>op \<in> set ops. op \<in> ((\<Pi>)\<^sub>\<O>)"
  proof -
    (* TODO refactor *)
      ...
      by (induction \<pi>; force)
    {
      ...
      moreover obtain op where "op \<in> set \<pi>" and "ops = [op]"
        ...
        by blast
      ultimately have "\<forall>op \<in> set ops. op \<in> ((\<Pi>)\<^sub>\<O>)"
        using assms(1)
        by simp
    }
    thus ?thesis..
...
theorem  embedding_lemma:
  ...
  proof  -
    (* TODO refactor \<open>STRIPS_Prime_Representation\<close> (characterization of valid operator).
  *)have nb\<^sub>1: "\<forall>op \<in> set \<pi>. op \<in> ((\<Pi>)\<^sub>\<O>)"
      using assms(2)
  ...
  *)      {
    ...
    moreover have "\<forall>ops \<in> set (embed \<pi>). \<forall>op \<in> set ops. op \<in> ((\<Pi>)\<^sub>\<O>)"
...
lemma flattening_lemma_i:
  fixes \<Pi>:: "'a strips_problem"
  assumes "\<forall>ops \<in> set \<pi>. \<forall>op \<in> set ops. op \<in> ((\<Pi>)\<^sub>\<O>)"
  shows "\<forall>op \<in> set (concat \<pi>). op \<in> ((\<Pi>)\<^sub>\<O>)"
  proof -
    {
      fix op
      ...
      then obtain ops where "ops \<in> set \<pi>" and "op \<in> set ops"
        using UN_iff
        by blast
      ultimately have "op \<in> ((\<Pi>)\<^sub>\<O>)"
        using assms
        by blast
    }
    thus ?thesis..
...
lemma flattening_lemma:
  ...
  proof  -
    let ?\<pi>' = "concat \<pi>"
    {
      have "\<forall>ops \<in> set \<pi>. \<forall>op \<in> set ops. op \<in> ((\<Pi>)\<^sub>\<O>)"
        using assms(3)
        unfolding is_parallel_solution_for_problem_def list_all_iff ListMem_iff
        by force
      hence "\<forall>op \<in> set ?\<pi>'. op \<in> ((\<Pi>)\<^sub>\<O>)"
...
lemma set_to_precondition_of_op_is[simp]: "set (to_precondition op)
  ...
  unfolding to_precondition_def STRIPS_Prime_Representation.to_precondition_def set_map
  by blast
