...
definition SAS_Plus_Plus_Prime_State_To_SAS_Plus:: 
  "('d domain_element \<times> ('v, 'd) state) \<Rightarrow> ('v variable, 'd domain_element) state" where
"SAS_Plus_Plus_Prime_State_To_SAS_Plus is = ((\<lambda>x. Some (DE x)) \<circ>\<^sub>m (snd is)
  \<circ>\<^sub>m (\<lambda>x. (case x of Var x \<Rightarrow> Some x | Stage \<Rightarrow> None)))(Stage \<mapsto> (fst is))"

lemma SAS_Plus_Plus_Prime_State_To_SAS_Plus_eq_iff_snd_eq[simp]:
  "SAS_Plus_Plus_Prime_State_To_SAS_Plus (i, s) = SAS_Plus_Plus_Prime_State_To_SAS_Plus (i, s') 
  \<longleftrightarrow> s = s'" 
  apply (auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_comp_def fun_eq_iff 
      split: option.splits variable.splits)
  by (metis option.exhaust_sel)

lemma SAS_Plus_Plus_Prime_State_To_SAS_Plus_Stage[simp]: 
  "SAS_Plus_Plus_Prime_State_To_SAS_Plus (i, s) Stage = Some i"
  by (auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def)

lemma SAS_Plus_Plus_Prime_State_To_SAS_Plus_update_stage[simp]:
  "SAS_Plus_Plus_Prime_State_To_SAS_Plus (a, s)(Stage \<mapsto> b) = SAS_Plus_Plus_Prime_State_To_SAS_Plus (b, s)"
  by (auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_comp_def fun_eq_iff)

lemma SAS_Plus_Plus_Prime_State_To_SAS_Plus_update_Var[simp]:
  "SAS_Plus_Plus_Prime_State_To_SAS_Plus (i, s)(Var x \<mapsto> DE y) 
    = SAS_Plus_Plus_Prime_State_To_SAS_Plus(i, s(x \<mapsto> y))"
  by (auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_comp_def fun_eq_iff 
      split: option.splits variable.splits)

lemma SAS_Plus_Plus_Prime_State_To_SAS_Plus_map_le[simp]: 
  "SAS_Plus_Plus_Prime_State_To_SAS_Plus (a, s) \<subseteq>\<^sub>m m 
    \<Longrightarrow> SAS_Plus_Plus_Prime_State_To_SAS_Plus (b, s) \<subseteq>\<^sub>m m(Stage \<mapsto> b)" 
  by(auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_le_def map_comp_def 
      split: option.splits variable.splits)

lemma Stage_map_le_SAS_Plus_Plus_Prime_State_To_SAS_Plus[simp]:
  "[Stage \<mapsto> i] \<subseteq>\<^sub>m SAS_Plus_Plus_Prime_State_To_SAS_Plus (i, s)"
  by(auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_le_def)

lemma Var_not_in_map_Var_iff[simp]: "Var x \<notin> Var ` S \<longleftrightarrow> x \<notin> S" by auto

lemma dom_of_SAS_Plus_Plus_Prime_State_To_SAS_Plus: 
  "dom (SAS_Plus_Plus_Prime_State_To_SAS_Plus (i, s)) = { Stage } \<union> (Var ` dom s)" 
  by(auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_comp_def 
      split: option.splits variable.splits)

lemma SAS_Plus_Plus_Prime_State_To_SAS_Plus_map_le_SAS_Plus_Plus_Prime_State_To_SAS_Plus[simp]:
  "SAS_Plus_Plus_Prime_State_To_SAS_Plus (a, s1) \<subseteq>\<^sub>m SAS_Plus_Plus_Prime_State_To_SAS_Plus (b, s2)
    \<longleftrightarrow> (a = b \<and> s1 \<subseteq>\<^sub>m s2)" 
  apply(auto simp: map_le_def dom_of_SAS_Plus_Plus_Prime_State_To_SAS_Plus)
   apply(auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_comp_def split: option.splits)
  by force+

definition SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator:: 
  "('v, 'd) sas_plus_operator \<Rightarrow> ('v, 'd) operator" where
"SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator op = 
...
lemma SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator_applicable[simp]: 
  "is_operator_applicable_in (SAS_Plus_Plus_Prime_State_To_SAS_Plus (i, s)) 
    (SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator op)
  \<longleftrightarrow> (i = NonInit \<and> is_operator_applicable_in s op)"
  apply(auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_comp_def 
      SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator_def)
...
lemma SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator_execute[simp]:
  "SAS_Plus_Plus_Prime_State_To_SAS_Plus (i, s) ++ map_of (effect_of (SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator op))
  = SAS_Plus_Plus_Prime_State_To_SAS_Plus(i, s \<then>\<^sub>+ op)" 
  using map_of_SomeD
  apply(auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_comp_def fun_eq_iff map_add_def 
      SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator_def split: option.splits variable.splits)
            apply force+
...
lemma execute_SAS_Plus_Plus_ops_in_SAS_Plus[simp]:
  "execute_serial_plan_sas_plus (SAS_Plus_Plus_Prime_State_To_SAS_Plus (NonInit, s))
    (map SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator ops) 
    = SAS_Plus_Plus_Prime_State_To_SAS_Plus (NonInit, execute_serial_plan_sas_plus s ops)"
  apply(induction ops arbitrary: s)
   apply(simp add: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def 
      SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator_def)
  by(auto simp: SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator_applicable[simplified])

(* TODO *)
(* For all undefined values, give a list of operators that assigns these values to every possible value. *)
(* Since the set by range_of might be infinite, it only makes sense if the list of operators is also infinite, hence a set. *)
definition initialization_operators:: "('v, 'd) sas_plus_problem \<Rightarrow> ('v, 'd) operator set" where
...
lemma in_initialization_operators_iff: 
  ...
  \<longleftrightarrow> (x \<in> set ((P)\<^sub>\<V>\<^sub>+) \<and> ((P)\<^sub>I\<^sub>+) x = None \<and> y \<in> (the (range_of P x)))"
...
lemma SAS_Plus_Plus_Prime_To_SAS_Plus_valid: 
  ...
  shows "is_valid_problem_sas_plus (SAS_Plus_Plus_Prime_To_SAS_Plus P)" 
  using assms apply(auto simp: is_valid_problem_sas_plus_plus_def is_valid_problem_sas_plus_def 
      SAS_Plus_Plus_Prime_To_SAS_Plus_def list_all_def is_valid_operator_sas_plus_def Let_def
      initialization_operators_def SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator_def ListMem_iff)
               apply auto
              apply fastforce
             apply fastforce
            apply fastforce
...
lemma stage_of_initial_state[simp]: "((SAS_Plus_Plus_Prime_To_SAS_Plus P)\<^sub>I\<^sub>+) Stage = Some Init" 
  by(auto simp: SAS_Plus_Plus_Prime_To_SAS_Plus_def initial_state_def)
...
lemma result_of_initialization_sequence[simp]: 
  ...
  I ++ SAS_Plus_Plus_Prime_State_To_SAS_Plus (Init, map_of vs)"
using assms proof (induction vs arbitrary: I)
  case Nil
  then show ?case using Nil
    by (auto simp: initialization_sequence_def SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_add_def)
next
  case (Cons v vs)
  ...
  also have "... = I ++ SAS_Plus_Plus_Prime_State_To_SAS_Plus (Init, map_of (v # vs))" using Cons
    by(auto simp: SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_add_def fun_eq_iff map_comp_def 
...
lemma SAS_Plus_Plus_Prime_To_SAS_Plus:
  assumes "is_valid_problem_sas_plus_plus P" 
...
lemma map_list_set_equivalent: 
  "set (map f bs) = f` (set bs)" by auto
...
lemma chain_applicable_in_Stage_NonInit:
  ...
  \<Longrightarrow> \<forall>op \<in> set plan. op \<in> SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator ` (P)\<^sub>\<O>\<^sub>+"
...
lemma SAS_plus_plan_structure:
  ...
  shows "\<exists>k. k < length plan 
    ...
    \<and> list_all (\<lambda>op. op \<in> (SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator `((P)\<^sub>\<O>\<^sub>+))) 
      (drop (k + 1) plan)"
using assms proof(induction plan arbitrary: s)
  ...
  hence "a \<in> (initialization_operators P) 
    ...
    \<or> a \<in> (SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator `((P)\<^sub>\<O>\<^sub>+))" 
  ...
  next
    ...
    hence "\<forall>op \<in> set plan. op \<in> (SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator `((P)\<^sub>\<O>\<^sub>+))" 
      apply -
  ...
  qed (auto simp: SAS_Plus_Plus_Prime_Operator_To_SAS_Plus_Operator_def list_all_def map_le_def)
qed auto
...
lemma result_of_initialization_operators_on_initial_state:
  "is_valid_problem_sas_plus_plus P
    ...
    \<Longrightarrow> (\<exists>I'. execute_serial_plan_sas_plus ((SAS_Plus_Plus_Prime_To_SAS_Plus P)\<^sub>I\<^sub>+) ops 
        = SAS_Plus_Plus_Prime_State_To_SAS_Plus (Init, I')
      \<and> ((P)\<^sub>I\<^sub>+) \<subseteq>\<^sub>m I' \<and> (\<forall>v \<in> dom I'. the (I' v) \<in> \<R>\<^sub>+ P v))"
  apply(simp add: SAS_Plus_Plus_Prime_To_SAS_Plus_def initial_state_def)
...
lemma SAS_Plus_To_SAS_Plus_Plus:
  assumes "is_valid_problem_sas_plus_plus P" 
    "is_serial_solution_for_problem (SAS_Plus_Plus_Prime_To_SAS_Plus P) plan"
...
proof-
  ...
  obtain k where k_def: "k < length ?plan 
    ...
    by(force simp: is_serial_solution_for_problem_def list_all_def map_le_def Let_def 
        execute_chain_applicable_prefix SAS_Plus_Plus_Prime_To_SAS_Plus_def initial_state_def ListMem_iff 
        SAS_Plus_Plus_Prime_State_To_SAS_Plus_def)+

  let ?prefix = "take k ?plan" 
  obtain I' where I'_def: "execute_serial_plan_sas_plus ((SAS_Plus_Plus_Prime_To_SAS_Plus P)\<^sub>I\<^sub>+) ?prefix 
        = SAS_Plus_Plus_Prime_State_To_SAS_Plus (Init, I')
      \<and> ((P)\<^sub>I\<^sub>+) \<subseteq>\<^sub>m I' \<and> (\<forall>v \<in> dom I'. the (I' v) \<in> \<R>\<^sub>+ P v)" 
    using result_of_initialization_operators_on_initial_state assms k_def by blast
  moreover have  "dom (execute_serial_plan_sas_plus ((SAS_Plus_Plus_Prime_To_SAS_Plus P)\<^sub>I\<^sub>+) ?prefix) 
    = set ((SAS_Plus_Plus_Prime_To_SAS_Plus P)\<^sub>\<V>\<^sub>+)" 
    apply(rule dom_of_execute_serial_plan_sas_plus)
    using SAS_Plus_Plus_Prime_To_SAS_Plus_valid assms k_def 
    by(auto simp: list_all_def SAS_Plus_Plus_Prime_To_SAS_Plus_def initial_state_def 
        SAS_Plus_Plus_Prime_State_To_SAS_Plus_def map_comp_def split: option.splits variable.splits)
  ...
  also have "... = SAS_Plus_Plus_Prime_State_To_SAS_Plus (NonInit, execute_serial_plan_sas_plus I' plan')"
  ...
  ultimately have "(P)\<^sub>G\<^sub>+ \<subseteq>\<^sub>m execute_serial_plan_sas_plus I' plan'"
    ...
    by(auto simp: execute_chain_applicable_prefix is_serial_solution_for_problem_def 
        SAS_Plus_Plus_Prime_To_SAS_Plus_def)
