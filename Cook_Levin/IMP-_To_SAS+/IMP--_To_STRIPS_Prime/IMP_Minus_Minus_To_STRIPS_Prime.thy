...
theorem imp_minus_minus_to_sas_plus_plus:
  assumes "(c, is1) \<rightarrow>\<^bsup>t\<^esup> (SKIP, is2)"
   ...
   "t \<le> t'"
 shows "\<exists>plan. (STRIPS_Prime_Semantics.is_serial_solution_for_problem (\<phi> (SAS_Plus_Plus_Prime_To_SAS_Plus(imp_minus_minus_to_sas_plus c I G))) plan) 
        \<and> length plan \<le> t' + length vs + 2" 
proof - 
  let ?P_sas2 = "imp_minus_minus_to_sas_plus c I G"
  let ?P_sas1 = "SAS_Plus_Plus_Prime_To_SAS_Plus ?P_sas2"
  from assms obtain plan2 where 1:"is_serial_solution_for_problem_sas_plus_plus ?P_sas2 plan2 
    \<and> length plan2 \<le> t'"
    by (meson imp_minus_minus_to_sas_plus_plus)
  from assms have 2:"is_valid_problem_sas_plus_plus ?P_sas2 " 
    by (simp add: imp_minus_minus_to_sas_plus_valid)
  with 1 obtain prefix where 3:"length prefix \<le> length ((?P_sas2)\<^sub>\<V>\<^sub>+) + 1" and 
    ...
    using SAS_Plus_Plus_Prime_To_SAS_Plus by blast
  ...
  then have 5: "length ((?P_sas2)\<^sub>\<V>\<^sub>+) = length (enumerate_variables c) + 1" 
    ...
    by (metis sas_plus_problem.select_convs(1))
