...
*)
theory SAS_Plus_Prime_Representation
imports "$AFP/Verified_SAT_Based_AI_Planning/State_Variable_Representation"
begin

section "SAS+' Representation"

text \<open> This is based on Verified_SAT_Based_AI_Planning.SAS_Plus_Representation. 
This is a version of SAS+ that do not require the range of a variable to be finite. 
More precisely, in sas_plus_problem.range_of the second type is changed from list to set. 
The definition of range_of' is fitted. Lemmas involving range_of is also fitted. 
Search for "HERE!" to see what was changed. \<close>
...
definition is_valid_operator_sas_plus
  ...
  where "is_valid_operator_sas_plus \<Psi> op \<equiv> let 
    ...
    in list_all (\<lambda>(v, a). ListMem v vs) pre
      \<and> list_all (\<lambda>(v, a). (D v \<noteq> None) \<and> a \<in> (the (D v))) pre
...
definition "is_valid_problem_sas_plus \<Psi> 
  \<equiv> let ops = operators_of \<Psi>
    ...
    \<and> (\<forall>v. G v \<noteq> None \<longrightarrow> (the (G v)) \<in> (the (D v)))" 
...
lemma[simp]: 
  ...
  unfolding initial_of_def goal_of_def variables_of_def range_of_def operators_of_def      
    SAS_Plus_Prime_Representation.is_operator_applicable_in_def
    SAS_Plus_Prime_Representation.execute_operator_sas_plus_def
  by simp+
...
lemma range_of_not_empty:
  ...
  by (auto simp add: SAS_Plus_Prime_Representation.range_of'_def) 
...
proof -
  ...
  moreover have "\<forall>(v, a) \<in> set ?pre. v \<in> set ((\<Psi>)\<^sub>\<V>\<^sub>+)"
    ...
    by blast+
  moreover {
    have "\<forall>(v, a) \<in> set ?pre. (?D v \<noteq> None) \<and> a \<in> (the (?D v))"
      using assms 
      unfolding is_valid_operator_sas_plus_def Let_def list_all_iff ListMem_iff
      by argo
  ...
  moreover {
    have "\<forall>(v, a) \<in> set ?eff. (?D v \<noteq> None) \<and> a \<in> (the (?D v))"
      using assms 
      unfolding is_valid_operator_sas_plus_def Let_def list_all_iff ListMem_iff
      by argo
...
proof -
  ...
  have nb\<^sub>1: "\<forall>v \<in> set ?vs. ?D v \<noteq> None"
    ...
    and nb\<^sub>3: "\<forall>v. ?G v \<noteq> None \<longrightarrow> the (?G v) \<in> (the (?D v))"
    using assms 
