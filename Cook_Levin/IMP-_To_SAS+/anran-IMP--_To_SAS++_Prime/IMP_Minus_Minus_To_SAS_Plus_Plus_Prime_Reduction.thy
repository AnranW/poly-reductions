...
theory IMP_Minus_Minus_To_SAS_Plus_Plus_Prime_Reduction 
  imports "IMP_Minus_Minus_To_SAS_Plus_Plus_Prime_State_Translations" "../IMP--_To_SAS++/IMP_Minus_Minus_Subprograms"
begin
...
(* This function maps an IMP- program to a set of SAS++' problems with all possible initial and final states. 
   In other words, the initial and final states are undefined. *)
...
proof (cases v)
  ...
  then have *: "(VN x, set domain) \<in> set (map (\<lambda> v. (VN v, set domain)) (enumerate_variables c))" 
    using assms by (auto simp: Let_def imp_minus_minus_to_sas_plus_def)
  then have "range_of (imp_minus_minus_to_sas_plus c I G) v = Some (set domain)" 
    using VN
