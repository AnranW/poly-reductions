...
text \<open> This is the corresponding semantics for SAS+'. Search for "HERE!" to see where was changed. \<close>
...
proof -
  have "are_all_operators_applicable_in s [op]"
    unfolding are_all_operators_applicable_in_def 
      ...
      is_operator_applicable_in_def SAS_Plus_Prime_Representation.is_operator_applicable_in_def
      list_all_iff  
    using assms(1) 
    by fastforce
...
proof (induction \<psi> arbitrary: I)
  ...
  show ?case 
    proof (cases "are_all_operators_applicable_in I [op]")
      ...
      have "SAS_Plus_Prime_Representation.is_operator_applicable_in I op"
        using True 
      ...
      moreover {
        have "is_operator_applicable_in I op" 
          ...
          unfolding are_all_operators_applicable_in_def 
            SAS_Plus_Prime_Representation.is_operator_applicable_in_def list_all_iff 
          by fastforce+
        hence "?J = ?J'" 
...
lemma is_valid_operator_sas_plus_then_range_of_sas_plus_op_is_set_range_of_op:
  ...
  shows "(\<R>\<^sub>+ \<Psi> v) = (the (sas_plus_problem.range_of \<Psi> v))" 
proof -
...
lemma set_the_range_of_is_range_of_sas_plus_if:
  ...
  shows " (the (sas_plus_problem.range_of \<Psi> v)) = \<R>\<^sub>+ \<Psi> v"
proof-
