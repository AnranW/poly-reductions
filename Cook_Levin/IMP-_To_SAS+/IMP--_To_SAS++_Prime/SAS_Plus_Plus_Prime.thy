...
text \<open> This is the corresponding version of SAS++ using SAS+' syntax and semantics. 
More about SAS++ see SAS_Plus_Plus.thy 
To see where was changed look for "HERE!". \<close>
text \<open> SAS++', like SAS++, allows for partial initial states, i.e. not all variables are assigned. \<close>
...
definition is_serial_solution_for_problem_sas_plus_plus_prime
  :: "('variable, 'domain) state \<Rightarrow> ('variable, 'domain) state \<Rightarrow> ('variable, 'domain) sas_plus_problem \<Rightarrow> ('variable, 'domain) sas_plus_plan \<Rightarrow> bool" 
  where "is_serial_solution_for_problem_sas_plus_plus_prime I G \<Psi> \<psi>
    \<equiv> let ops = sas_plus_problem.operators_of \<Psi>
      in (\<exists>I'. I \<subseteq>\<^sub>m I' \<and> dom I' = set ((\<Psi>)\<^sub>\<V>\<^sub>+)
        \<and> (\<forall> v \<in> set ((\<Psi>)\<^sub>\<V>\<^sub>+). the (I' v) \<in> range_of' \<Psi> v)
        \<and> G \<subseteq>\<^sub>m execute_serial_plan_sas_plus I' \<psi> 
        \<and> list_all (\<lambda>op. op \<in> ops) \<psi>)" 
